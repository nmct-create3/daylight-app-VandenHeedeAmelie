...
const updateSun = (leftOffset, bottemOffset) => {
  ...
  htmlSun.setAttribute('data-time', new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }));
};
...
let getAPI = async (lat, lon) => {
  const weatherInfo = await fetch(`http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=5c3685a69a192d16b52715fb86e4a277&units=metric&lang=nl&cnt=1`).then((response) => response.json());
